...
spin3 <- spin3d(c(0,1,0), rpm = 1/p.moon ) # the sprite spinner --- moon
spriteid <- NULL
...
# loc1=PCS2CCS(-readDay * 360 / p.earth, a = pe$a, ab=pe$a/pe$b)
# loc2=PCS2CCS(-readDay * 360 / p.moon, a = pm$a, ab=pm$a/pm$b)
...
f <- function(time, fps) {
  ...
  plot3d(orb.moon, type='n', col = 1, ylim=range(orb.moon[,2])+c(-1,1)*20,
         xlim=range(orb.moon[,1])+c(-1,1)*120, axes=F, xlab='', ylab='', zlab='');
  # bg3d('black')
         #, xlab='XXX', ylab='ylab', zlab='zlab')
  ...
  obj.sun <- spheres3d(c(0,0,0), radius=rr[1], col='transparent',
  ...
  obj.moon <-spheres3d(c(0,0,0),radius=rr[3], col = cols[icol],
                       texture = system.file('extdata/moon.png', package = 'RoundAndRound') )

  # loc1=PCS2CCS(-readDay / p.earth * 360, a = pe$a, ab=pe$a/pe$b)
  # loc2=PCS2CCS(-readDay / p.moon * 360, a = pm$a, ab=pm$a/pm$b)
  loc1 = orb.earth[time, ]
  loc2 = orb.moon[time, ]
  loc1 = TD2TD(loc1)
  loc2 = TD2TD(loc2)
